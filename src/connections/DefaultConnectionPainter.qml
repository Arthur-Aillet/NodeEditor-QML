...
AbstractConnectionPainter {
    ...
    property color inTypeColor: {
        ...
        return style.typeColor(dataType.id);
    }
    ...
    override property rect boundingBox: {
        const bbx = Math.min(obj.inPoint.x, obj.outPoint.x, c1.x, c2.x) - style.lineWidth;
        const bby = Math.min(obj.inPoint.y, obj.outPoint.y, c1.y, c2.y) - style.lineWidth;

        return {
            x: bbx,
            y: bby,
            width: (Math.max(obj.inPoint.x, obj.outPoint.x, c1.x, c2.x) - bbx) + style.lineWidth * 3,
            height: (Math.max(obj.inPoint.y, obj.outPoint.y, c1.y, c2.y) - bby) + style.lineWidth * 3
        };
    }
    ...
    function innerStrokeStyle(ctx): color {
        ...
        for (let i = 0; i != 10; i++) {
            const color = style.lerpOklabColors(inTypeColor, outTypeColor, i / 10);
    ...
    onPaint: {
        ...
        if (obj.fullyConnected) {
            ...
            ctx.lineWidth = style.lineWidth * 0.45 * scaleFactor;
            ctx.stroke();
        } else {
            ctx.setLineDash([6, 2]);
            ctx.lineWidth = style.constructionLineWidth * scaleFactor;
