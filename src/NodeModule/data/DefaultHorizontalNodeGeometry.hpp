...
class DefaultHorizontalNodeGeometry : public AbstractNodeGeometry {
  ...
  QPointF portPosition(NodeId const nodeId, PortType const portType,
                       PortIndex const index) const override;

  QPointF portTextPosition(NodeId const nodeId, PortType const portType,
  ...
  private:
  QRectF portTextRect(NodeId const nodeId, PortType const portType,
